...
foreach($daten as $xpos => $datensatz) {
   foreach ($datensatz as $ypos => $datensatz2) {
      ...
      $color=imagecolorallocate($image,$intensity,$intensity,$intensity);
		...
		$map_data.='\', CAPTION, \'Spieler:\', WIDTH, 300, FGCOLOR, \'#ffffff\', TEXTCOLOR, \'#ffffff\', FGBACKGROUND,\'http://www.stfc.it:82/stgc5_gfx/skin1/bg_stars1.gif\', BGCOLOR, \'#687b88\', BORDER, 2, CAPTIONFONT, \'Arial\', CAPTIONSIZE, 2, TEXTFONT, \'Arial\', TEXTSIZE, 2);" onmouseout="return nd();">
		';
	}
}
...
imagepng($image_small,$game_path . "bbkarte_thumb.png");
imagepng($image,$game_path . "bbkarte.png");


$fp=fopen ($game_path . 'bbkarte.htm','w');
fputs($fp,$map_data);
fclose($fp);
