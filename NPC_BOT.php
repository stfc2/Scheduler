...
class NPC
{
	...
	function MessageUser($sender,$receiver, $header, $message)
	{
		...
		$game = new game();
		if ($db->query('INSERT INTO message (sender, receiver, subject, text, rread,time) VALUES ('.$sender.',"'.$receiver.'","'.addslashes($header).'","'.addslashes($message).'",0,'.$game->TIME.')')==false)
		{
			$sdl->log('systemmessage_query: Could not call INSERT INTO in message :-:INSERT INTO message (sender, receiver, subject, text, rread,time)
				VALUES ('.$sender.',"'.$receiver.'","'.addslashes($header).'","'.addslashes($message).'",0, '.$game->TIME.')', TICK_LOG_FILE_NPC);
		}
		$sdl->log("Senderid:".$sender." // Receiverid:".$receiver, TICK_LOG_FILE_NPC);
		$num=$db->queryrow('SELECT COUNT(id) as unread FROM message WHERE (receiver="'.$receiver.'") AND (rread=0)');
		if($num['unread']>0) $db->query_a('UPDATE user SET unread_messages="'.$num['unread'].'" WHERE user_id="'.$receiver.'"');
		$sdl->log("Num:".$num['unread'], TICK_LOG_FILE_NPC);
	...
	public function NPC($debuggen=0,$title="",$type=0,$color="#ffffff")
	{
		...
		$debug_array_logen=0;
		$debug_sql_logen=0;
		if($debug_zu= $db->queryrow('SELECT * FROM FHB_debug LIMIT 0,1'))
		{
		...
		$Umwelt = $db->queryrow('SELECT * FROM config LIMIT 0 , 1');
		$ACTUAL_TICK = $Umwelt['tick_id'];
		$STARDATE = $Umwelt['stardate'];
		$sdl->start_job('Ramona basic system is checking if something is missing', TICK_LOG_FILE_NPC);
		//Erst feststellung ob der Bot schon ein exitenz besitzt
		if($Umwelt)
		{
			$sdl->log("The conversation with Ramona begins, oh, it is not beautiful, and then, it has such a great personality", TICK_LOG_FILE_NPC);
			$Bot_exe=$db->query('SELECT * FROM FHB_Bot LIMIT 0,1');
			$num_bot=$db->num_rows($Bot_exe);
			if($num_bot<1)
			{
				...
				if(!$db->query($sql))
				{
					$sdl->log('<b>Error:</b> Abort the program because of errors when creating the user', TICK_LOG_FILE_NPC);
					exit;
				}
			}
			//So jetzt geben wir dem Bot mal ein paar Daten damit er auch Registriert ist
			$this->bot = $db->queryrow('SELECT * FROM FHB_Bot LIMIT 0,1');
			//Schauen ob der Bot schon lebt
			if($this->bot['user_id']==0){
				$sdl->log('Ramona is created', TICK_LOG_FILE_NPC);
				...
				if(!$db->query($sql))
				{
					$sdl->log('<b>Error:</b> Bot: Could not create Ramona', TICK_LOG_FILE_NPC);
				}else{
					$sql = 'Select * FROM user WHERE user_name="Quark(NPG)" and user_loginname="Bot" and user_auth_level=1';
					$Bot_zw = $db->queryrow($sql);
					if(!$Bot_zw['user_id'])
					{
						$sdl->log('<b>Error:</b> The variable $Bot_zw has no content', TICK_LOG_FILE_NPC);
						//break;
					}
					...
					if(!$db->query($sql)) {
						$sdl->log('<b>Error:</b> Bot card: Could not change the card', TICK_LOG_FILE_NPC);
					}
					$this->bot = $db->queryrow('SELECT * FROM FHB_Bot');
				}
			}
			//The bot should also have a body of what looks
			if($this->bot['planet_id']==0){
				$sdl->log('<b>Ramona needs a new body</b>', TICK_LOG_FILE_NPC);
				while($this->bot['planet_id']==0 or $this->bot['planet_id']=='empty'){
					$sdl->log('New planet', TICK_LOG_FILE_NPC);
					$db->lock('starsystems_slots');
					$this->bot['planet_id']=create_planet($this->bot['user_id'], 'quadrant', 4);
					$db->unlock();
					if($this->bot['planet_id'] == 0)
					{
						$sdl->log('<b>Error:</b> Bot Planet id doesn\'t go', TICK_LOG_FILE_NPC);
						exit;
					}
					...
					if(!$db->query($sql)) {
						$sdl->log('<b>Error:</b> Bot body: Planet has not been created', TICK_LOG_FILE_NPC);
					}else{
						//Bot bekommt bessere werte für seinen Körper, er soll ja auch gut aussehen
						$sdl->log('Better values for the Planet', TICK_LOG_FILE_NPC);
						...
						if(!$db->query($sql)) $sdl->log('<b>Error:</b> Bot body: the body could not be improved', TICK_LOG_FILE_NPC);
						$sql = 'UPDATE FHB_Bot SET planet_id='.$this->bot['planet_id'].' WHERE user_id = '.$this->bot['user_id'];
						if(!$db->query($sql)) $sdl->log('<b>Error:</b> Bot card: could not change planet info card', TICK_LOG_FILE_NPC);
					}
				}
			}
			...
			if($this->bot['ship_t_1']==0)
			{
				...
				if(!$db->query($sql))  $sdl->log('<b>Error:</b> Bot ShipsTemps: template 1 was not saved', TICK_LOG_FILE_NPC);
			}
			if($this->bot['ship_t_2']==0)
			{
				...
				if(!$db->query($sql))  $sdl->log('<b>Error:</b> Bot ShipsTemps: template 2 was not saved', TICK_LOG_FILE_NPC);
			}
			if($neuladen>0)
			{
				$sdl->log('Build ship templates', TICK_LOG_FILE_NPC);
				$Bot_temps=$db->query('SELECT id FROM ship_templates s WHERE owner='.$this->bot['user_id']);
				//$Bot_temps=$db->queryrow('SELECT id FROM ship_templates s WHERE owner='.$this->bot['user_id'].'');
				$zaehler_temps=0;
				$Bot_neu = array();

				$tempID = $db->fetchrow($Bot_temps);
				$Bot_neu[0]=$tempID['id'];
				$tempID = $db->fetchrow($Bot_temps);
				$Bot_neu[1]=$tempID['id'];
...
//				$Bot_neu[1]=$Bot_temps[1]['id'];
				...
				if(!$db->query($sql))  $sdl->log('<b>Error:</b> Bot ShipsTemps: could not save the template id', TICK_LOG_FILE_NPC);
			}
		}else{
			$sdl->log('<b>Error:</b> No access to the bot table=>'.$Bot_exe, TICK_LOG_FILE_NPC);
			exit;
		}
		$sdl->finish_job('Basic System', TICK_LOG_FILE_NPC);
		// ########################################################################################
		// ########################################################################################
		//PW des Bot änderns - nix angreifen
		$sdl->start_job('PW change', TICK_LOG_FILE_NPC);
		$wert_new=rand(1,9);
		$zufall="vv";
		...
		if($db->query('UPDATE `user` SET `user_password`=MD5("'.$wert_new.'") WHERE `user_id` ='.$this->bot['user_id'].''))
		{
			$sdl->log('Now there are only One-Night-Stands, no longer relations', TICK_LOG_FILE_NPC);
		}
		$sdl->finish_job('PW change', TICK_LOG_FILE_NPC);
		// ########################################################################################
		// ########################################################################################
		// Messages answer
		$sdl->start_job('Messages answer', TICK_LOG_FILE_NPC);
		$anzahl_igms=0;
		$sql = 'SELECT * FROM message WHERE receiver='.$this->bot['user_id'].' AND rread=0';
		if(!$q_message = $db->query($sql))
		{
			$sdl->log('<b>Error:</b> IGM: Could not query messages', TICK_LOG_FILE_NPC);
		}else{
			while($message = $db->fetchrow($q_message))
			{
				...
				if(!($language = $db->queryrow($sql)))
				{
					$sdl->log('<b>Error:</b> Cannot read user language!', TICK_LOG_FILE_NPC);
				...
				switch($language['language'])
				{
					case 'GER':
						...
						$title = '<b>Antwort auf ihr Schreiben</b>';
					break;
					case 'ENG':
						...
						$title = '<b>In reply to your letter</b>';
					break;
					case 'ITA':
						...
						$title = '<b>Risposta alla sua lettera</b>';
					break;
				}
		...
		if(!$db->query($sql))$sdl->log('<b>Error:</b> Message could not set to read', TICK_LOG_FILE_NPC);
		$sdl->log('Number of messages:'.$anzahl_igms, TICK_LOG_FILE_NPC);
		$sdl->finish_job('Messages answer', TICK_LOG_FILE_NPC);

		// ########################################################################################
		// ########################################################################################
		// Shiptrade Scheduler
		//So damit die Truppen dann auch wirklich bezhalt werden müssen, jetzt muss es nur noch tapsicher gemacht werden
		$sdl->start_job('Shiptrade Scheduler', TICK_LOG_FILE_NPC);
		$alpha_zaehler='0';
		...
		if(($q_strade = $db->query($sql)) === false) {
			$sdl->log('<b>Error:</b> Could not query scheduler shiptrade data! CONTINUED', TICK_LOG_FILE_NPC);
		}
		else
		{
			$n_shiptrades = 0;
			while($tradedata = $db->fetchrow($q_strade)) 
			{
				...
				{
					$sdl->log('shiptrade['.$tradedata['id'].'] had no bids', TICK_LOG_FILE_NPC);
					// Seller:
					$log_data=array(
						'player_name' => '',
					...
					if ($db->query('UPDATE ships SET ship_untouchable=0 WHERE ship_id='.$tradedata['ship_id'])!=true)
						$sdl->log('<b>Error (Critical)</b>: shiptrade['.$tradedata['id'].']: failed to free ship['.$tradedata['ship_id'].']!', TICK_LOG_FILE_NPC);
				}
				else
				{
					$sdl->log('Trade id: '.$tradedata['id'], TICK_LOG_FILE_NPC);
					...
					else
					{
						...
						$sdl->log('Last Bid:'.$last_bid['max_bid'].' and Prelastbid:'.$prelast_bid['max_bid'], TICK_LOG_FILE_NPC);
					...
					{
						$sdl->log('<b>Error:</b> shiptrade['.$tradedata['id'].'] had '.$tradedata['num_bids'].' bids but didn\'t find end_price', TICK_LOG_FILE_NPC);
					}
					else
					{
						// Seller:
						...
						if(!add_logbook_entry($tradedata['user'], LOGBOOK_AUCTION_VENDOR, $log_title, $log_data))
							$sdl->log('<b>Error:</b> Logbook could not be written - '.$tradedata['user'], TICK_LOG_FILE_NPC);;
						...
						if(!add_logbook_entry($hbieter['user_id'], LOGBOOK_AUCTION_PURCHASER, $log_title, $log_data))
							$sdl->log('<b>Error:</b> Logbook could not be written - '.$hbieter['user_id'], TICK_LOG_FILE_NPC);
						$sdl->log('Bidder:'.$hbieter['user_id'].' and Seller:'.$tradedata['user'], TICK_LOG_FILE_NPC);
						...
						if(!$db->query($sql))
						{
							$sdl->log('<b>Error: (Critical)</b> Debts were not saved: "'.$sql.'" - TICK EXECUTION CONTINUED', TICK_LOG_FILE_NPC);
						}else{
							$code=$db->insert_id();
							$sdl->log('[Code]: '.$code.'', TICK_LOG_FILE_NPC);
							$sql='INSERT INTO treuhandkonto (code,timestep) VALUES ('.$code.','.$ACTUAL_TICK.')';
							if(!$db->query($sql)) $sdl->log('<b>Error:</b> <b>Auction</b>Could not set number -'.$code, TICK_LOG_FILE_NPC);
							$sql='SELECT * FROM treuhandkonto WHERE code='.$code.'';
							$Auktionsnummer = $db->queryrow($sql);
							...
							$sdl->log('Ramona gets a ship ('.$plani.')...'.$sql, TICK_LOG_FILE_NPC);	
							if(!$db->query($sql))
							{
								$sdl->log('<b>Error: (Critical)</b>Could not give the ship to the Bot - '.$sql, TICK_LOG_FILE_NPC);
							}else{
								$sdl->log('shiptrade['.$tradedata['id'].'], processed sucessfully', TICK_LOG_FILE_NPC);
							}
						}
					}
				}
			...
			$sql = 'UPDATE ship_trade SET scheduler_processed=1
				...
				LIMIT '.$n_shiptrades;
			if(!$db->query($sql)) {
				$sdl->log('<b>Error: (Critical)</b> Could not update scheduler_shiptrade data - TICK EXECUTION CONTINUED -'.$sql, TICK_LOG_FILE_NPC);
			}
			unset($tradedata);
		}
		$sdl->finish_job('Shiptrade Scheduler', TICK_LOG_FILE_NPC);

		// ########################################################################################
		// ########################################################################################
		// Trust Account monitor
		$sdl->start_job('Trust Account monitor', TICK_LOG_FILE_NPC);
		$schulden_bezahlt=0;
		$schuldner=0;
		$spassbieter=0;
		$nachrichten_a =0;
		...
		$zeit_raum = 20*24*6;
		$zeit_raum_h = 20*24*3;
		$sql_a=$db->query('SELECT * FROM schulden_table WHERE status="0"');
		if(0<$db->num_rows($sql_a))
		{
			$sdl->log('Examine debts....', TICK_LOG_FILE_NPC);

			$sql = 'SELECT * FROM schulden_table WHERE status=0';
			if(($handel = $db->query($sql)) === false) {
				$sdl->log('<b>Error:</b> Could not query scheduler shiptrade data! CONTINUED //'.$sql, TICK_LOG_FILE_NPC);
			}else{
				$treffera=$db->num_rows();
				while($schulden= $db->fetchrow($handel))
				{
					if($schulden['id']==null)
					{
						$sdl->log('<b>Error:</b>No ID available', TICK_LOG_FILE_NPC);
					}else{
						$sql = $db->query('SELECT * FROM treuhandkonto WHERE code="'.$schulden['id'].'"');
						$treffer=$db->num_rows();
						if($treffer>1){
							$sdl->log('<b>(Error:1000)Programming error:</b>Apparently, there are several escrow accounts on the same schulden_table --'.$sql, TICK_LOG_FILE_NPC);
						}else if($treffer<=0){
							$sdl->log('<b>(Error:2000)Bug:</b>Apparently, there is no escrow account to the schulden_table -- '.$sql, TICK_LOG_FILE_NPC);
						}else if($treffer==1){
							$treuhand=$db->fetchrow($sql);
							...
							if($wert_ende==1)
							{
								...
								$sdl->log('--//--||=Everything not paid=||--\\--||'.$schulden['user_kauf'].'||'.$treuhand['code'].'||', TICK_LOG_FILE_NPC);
								if($treuhand['timestep']!=$schulden['timestep'])
								{
									$sdl->log('<b>(Error:3000)</b> - Someone having '.$schulden['user_kauf'].' with code '.$schulden['id'].'/'.$treuhand['code'].' have different time steps', TICK_LOG_FILE_NPC);
								}else {
									if(($treuhand['timestep']+$zeit_raum_h)<=$ACTUAL_TICK && ($treuhand['timestep']+$zeit_raum)>$ACTUAL_TICK && $schulden['mahnung']==0)
									{
										...
										$schuldner++;
										$User_kauf_V = $db->queryrow('SELECT user_id,user_trade FROM user WHERE user_id="'.$schulden['user_kauf'].'"');
										$db->query('UPDATE user SET user_trade="'.($User_kauf_V['user_trade']+1).'" WHERE user_id="'.$User_kauf_V['user_id'].'"');
										...
										if(!$db->query('UPDATE schulden_table SET mahnung=mahnung+1 WHERE id="'.$treuhand['code'].'" and user_ver="'.$schulden['user_ver'].'"')) $sdl->log('<b>Error:</b> Could not write warning -- UPDATE schulden_table SET mahnung=mahnung+1 WHERE id="'.$treuhand['code'].'" and user_ver="'.$schulden['user_ver'].'"', TICK_LOG_FILE_NPC);
									}
									if($ACTUAL_TICK>=($zeit_raum+$treuhand['timestep']))
									{
										...
										if(!$db->query($sql_1)) 	$sdl->log('<b>(Error:5000)ERROR-DELETE</b> <>'.$sql_1.'', TICK_LOG_FILE_NPC);
										$sql_2 = 'DELETE FROM treuhandkonto WHERE code="'.$schulden['id'].'"';
										if(!$db->query($sql_2)) 	$sdl->log('<b>(Error:5000)ERROR-DELETE</b> <>'.$sql_2.'', TICK_LOG_FILE_NPC);


										$sql_1='INSERT INTO FHB_sperr_list VALUES(null,'.$schulden['user_kauf'].','.$ACTUAL_TICK.')';
										
										if(!$db->query($sql_1)) $sdl->log('<b>No entry/b>User:'.$schulden['user_kauf'].' - got no further User Trade <>'.$sql_x.'', TICK_LOG_FILE_NPC);
										...
										$sdl->log('User: '.$schulden['user_ver'].' bekommt sein Schiff: '.$schulden['ship_id'], TICK_LOG_FILE_NPC);
										$sql_c='INSERT INTO `FHB_warteschlange` VALUES (NULL , '.$schulden['user_ver'].', '.$schulden['ship_id'].')';
										if(!$db->query($sql_c))   $sdl->log('<b>Error: (Critical)</b>could not put ship in the queue //'.$sql_c, TICK_LOG_FILE_NPC);
										$nachrichten_a++;	
										$user_name=$db->queryrow('SELECT user_name FROM user WHERE user_id='.$schulden['user_kauf'].'');
							...
							elseif($wert_ende==0)
							{
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['unit_1'].'||'.$schulden['unit_1'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['unit_2'].'||'.$schulden['unit_2'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['unit_3'].'||'.$schulden['unit_3'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['unit_4'].'||'.$schulden['unit_4'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['unit_5'].'||'.$schulden['unit_5'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['unit_6'].'||'.$schulden['unit_6'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['ress_1'].'||'.$schulden['ress_1'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['ress_2'].'||'.$schulden['ress_2'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$treuhand['ress_3'].'||'.$schulden['ress_3'].'||', TICK_LOG_FILE_NPC);
								$sdl->log('!!!||=Comparison--\\--'.$schulden['id'].'||'.$treuhand['code'].'||', TICK_LOG_FILE_NPC);
								//Schulden auf fertig stellen und sichtbar machen
								$schulden_bezahlt++;
								$sql_1 ='UPDATE schulden_table SET status="1" WHERE id='.$treuhand['code'].'';
								if(!$db->query($sql_1)) {
									$sdl->log('<b>Error:</b> schulden_table, was at '.$sql.' = status will not change', TICK_LOG_FILE_NPC);
								}
								...
								//Ship dem user geben
								$sdl->log('User: '.$schulden['user_kauf'].' get a ship: '.$schulden['ship_id'], TICK_LOG_FILE_NPC);
			...
			$sdl->log('[MESSAGE]No debts available', TICK_LOG_FILE_NPC);
		}
		//Leere Konton destroyen
		if(($konton_destroy=$db->query('SELECT * FROM schulden_table WHERE status=2'))==true)
		{
			while($konton_destroy_t=$db->fetchrow($konton_destroy))
			{
				...
				if(!$db->query($sql_e))
				{
					$sdl->log('<b>Error: (Critical)</b>Could not delete entry in schulden_table //Code:'.$konton_destroy_t['id'].'//'.$sql_c, TICK_LOG_FILE_NPC);
				}else{
					$sql_d='DELETE FROM treuhandkonto WHERE code="'.$konton_destroy_t['id'].'"';
					if(!$db->query($sql_d))
					{
						$sdl->log('<b>Error: (Critical)</b>Could not delete entry in treuhandkonto //Code:'.$konton_destroy_t['id'].'//'.$sql_c, TICK_LOG_FILE_NPC);
					}
				}
			}
		}
		else{$sdl->log('[Empties accounts] Accounts query was not || '.$konton_destroy.' = status will not change', TICK_LOG_FILE_NPC);}
		$sdl->log('Paid debt: '.$schulden_bezahlt, TICK_LOG_FILE_NPC);
		$sdl->log('Number of debtors: '.$schuldner, TICK_LOG_FILE_NPC);
		$sdl->log('Deleted account: '.$schuldner, TICK_LOG_FILE_NPC);
		$sdl->log('Number of fun bidder: '.$spassbieter, TICK_LOG_FILE_NPC);
		$sdl->log('Number of messages: '.$nachrichten_a++, TICK_LOG_FILE_NPC);
		$sdl->finish_job('Trust Account monitor', TICK_LOG_FILE_NPC);	
		// ########################################################################################
		// Börsenkurse ausrechenen
		// Where tick und Klasse
		$sdl->start_job('Stock trading ship', TICK_LOG_FILE_NPC);
		$sql_1 = array();
		$sql_2 = array();
		$sql_3 = array();
		...
		$sdl->log('Actual Tick:'.$ACTUAL_TICK.' -- '.$new_preis.' -- Period of:'.$min_tick.'', TICK_LOG_FILE_NPC);

		if($new_preis=='true')
		{
			$sdl->log('New graph is made.....', TICK_LOG_FILE_NPC);
			include("simple_graph.class.php");
			exec('cd '.FILE_PATH_hg.'kurs/; rm *.png');

			$sdl->start_job('Purchase - Unit', TICK_LOG_FILE_NPC);
			...
			$sdl->finish_job('Purchase - Unit', TICK_LOG_FILE_NPC);
		}
		$sdl->finish_job('Stock trading ship', TICK_LOG_FILE_NPC);
		// ########################################################################################
		// ########################################################################################
		//User freigeben fürs HZ
		$sdl->start_job('User release', TICK_LOG_FILE_NPC);
		$sql = "SELECT user_id,user_trade,trade_tick FROM user WHERE user_trade>0 AND trade_tick<=".$ACTUAL_TICK." AND trade_tick!=0";
		if(!$temps=$db->query($sql)) $sdl->log('<b>Error:</b> User query went wrong -- '.$sql, TICK_LOG_FILE_NPC);
		$anzahl_freigeben=0; 
		while($result = $db->fetchrow($temps))
		{
			...
			$sdl->log('User:'.$result['user_id'].' got the freedom to the women of this Galaxy fear to empty', TICK_LOG_FILE_NPC);
			if(!$db->query($sql_x)) $sdl->log('<b>Error:</b> update of the user --'.$sql_x, TICK_LOG_FILE_NPC);
			$anzahl_freigeben++;
		...
		$sdl->log('There were '.$anzahl_freigeben.' user released', TICK_LOG_FILE_NPC);
		$sdl->finish_job('User release', TICK_LOG_FILE_NPC);
		// ########################################################################################
		// ########################################################################################
		//User sperren fürs HZ
		$sdl->start_job('User lock', TICK_LOG_FILE_NPC);
		...
		while($result = $db->fetchrow($temps))
		{
			...
			if($result['anzahl']>0)
			{
				...
				if(!($sql_abfrage_1=$db->queryrow($sql_abfrage_1))) $sdl->log('<b>Error:</b> User query went wrong -- '.$sql_abfrage_1, TICK_LOG_FILE_NPC);
				...
				if($sql_abfrage_1['user_trade']<$result['anzahl'] && $sql_abfrage_1['trade_tick']!=0)
				{
					...
					$sdl->log('User:'.$result['user_id'].' has gotten a higher punishment - We, which was done to those injustice, are bad', TICK_LOG_FILE_NPC);
					if(!$db->query($sql_x)) $sdl->log('<b>Error:</b> User '.$result['user_id'].' cannot be locked for '.$sperre.' ticks</b>', TICK_LOG_FILE_NPC);
				...
				}else if($sql_abfrage_1['user_trade']<$result['anzahl'] && $sql_abfrage_1['trade_tick']==0)
				{
					...
					$sdl->log('User:'.$result['user_id'].' was banned - which he will meet his doom', TICK_LOG_FILE_NPC);
					if(!$db->query($sql_x)) $sdl->log('<b>Error:</b> User '.$result['user_id'].' cannot be locked for '.($sperre*3).' minutes.</b>', TICK_LOG_FILE_NPC);
		...
		$sdl->log('User '.$user_liste.' have their penalties and let me turn my problem in women clarify matters.', TICK_LOG_FILE_NPC);
		$sdl->log('There were '.$anzahl_sperren.' User locked', TICK_LOG_FILE_NPC);
		$sdl->finish_job('User locks', TICK_LOG_FILE_NPC);
		// ########################################################################################
		// ########################################################################################
		 ...
		 **
		 ****/
		$sdl->start_job('Update Ramona resources svailability', TICK_LOG_FILE_NPC);
		...
		$resources = $db->queryrow($sql);

		$sdl->log('Available resources: '.$resources['resource_1'].' -- '.$resources['resource_2'].' -- '.$resources['resource_3'], TICK_LOG_FILE_NPC);

		$sdl->log('Available units: '.$resources['unit_1'].' -- '.$resources['unit_2'].' -- '.$resources['unit_3'].' -- '.$resources['unit_4'].' -- '.$resources['unit_5'].' -- '.$resources['unit_6'], TICK_LOG_FILE_NPC);
		...
		if(!($tradecenter = $db->queryrow($sql)))
		{
			$sdl->log('<b>Warning:</b> Table FHB_Handels_Lager was empty! CONTINUED', TICK_LOG_FILE_NPC);
			...
			if(!$db->query($sql)) {
				$sdl->log('<b>Error:</b> Could not insert Handelslager - '.$update_res, TICK_LOG_FILE_NPC);
			}
		}
		else
		{
			...
			$sdl->log('Add units: '.$pick_u[0].' -- '.$pick_u[1].' -- '.$pick_u[2].' -- '.$pick_u[3].' -- '.$pick_u[4].' -- '.$pick_u[5], TICK_LOG_FILE_NPC);
			...
			if ($pick_u[0] > 0) {
				...
				if($metal_cost > $tradecenter['ress_1']  || $mineral_cost > $tradecenter['ress_2'] )
					...
					$sdl->log('Resources for Lvl 1 Soldiers: Metals '.$metal_cost.' Minerals '.$mineral_cost, TICK_LOG_FILE_NPC);
					$pick_r[0] += $metal_cost;
			...
			if ($pick_u[1] > 0) {
				...
				if($metal_cost > $tradecenter['ress_1']  || $mineral_cost > $tradecenter['ress_2'] )  
					...
					$sdl->log('Resources for Lvl 2 Soldiers: Metals '.$metal_cost.' Minerals '.$mineral_cost, TICK_LOG_FILE_NPC);
					$pick_r[0] += $metal_cost;
					$pick_r[1] += $mineral_cost;
				}
			}			
			...
			if ($pick_u[2] > 0) {
				...
				if($metal_cost > $tradecenter['ress_1']  || $mineral_cost > $tradecenter['ress_2']  || $dilithium_cost > $tradecenter['ress_3'] )
					...
					$sdl->log('Resources for Lvl 3 Soldiers: Metals '.$metal_cost.' - Minerals '.$mineral_cost.' - Dilithium '.$dilithium_cost, TICK_LOG_FILE_NPC);
					$pick_r[0] += $metal_cost;
					$pick_r[1] += $mineral_cost;
			...
			if ($pick_u[3] > 0) {
				...
				if($metal_cost > $tradecenter['ress_1']  || $mineral_cost > $tradecenter['ress_2']  || $dilithium_cost > $tradecenter['ress_3'] )
					...
					$sdl->log('Resources for Captains: Metals '.$metal_cost.' - Minerals '.$mineral_cost.' - Dilithium '.$dilithium_cost, TICK_LOG_FILE_NPC);
					$pick_r[0] += $metal_cost;
					$pick_r[1] += $mineral_cost;
					$pick_r[2] += $dilithium_cost;
				}
			...
			if ($pick_u[4] > 0) {
				...
				if($metal_cost > $tradecenter['ress_1']  || $mineral_cost > $tradecenter['ress_2']  || $dilithium_cost > $tradecenter['ress_3'] )
					...
					$sdl->log('Resources for Techs: Metals '.$metal_cost.' - Minerals '.$mineral_cost.' - Dilithium '.$dilithium_cost, TICK_LOG_FILE_NPC);
					$pick_r[0] += $metal_cost;
					$pick_r[1] += $mineral_cost;
					$pick_r[2] += $dilithium_cost;
				}
			...
			if ($pick_u[5] > 0) {
				...
				if($metal_cost > $tradecenter['ress_1']  || $mineral_cost > $tradecenter['ress_2']  || $dilithium_cost > $tradecenter['ress_3'] ) 
					...
					$sdl->log('Resources for Docs: Metals '.$metal_cost.' - Minerals '.$mineral_cost.' - Dilithium '.$dilithium_cost, TICK_LOG_FILE_NPC);
					$pick_r[0] += $metal_cost;
					$pick_r[1] += $mineral_cost;
					$pick_r[2] += $dilithium_cost;
				}
			...
			$sdl->log('Add resources: '.$pick_r[0].' -- '.$pick_r[1].' -- '.$pick_r[2], TICK_LOG_FILE_NPC);
			*/
			$sdl->log('Picking resources from CC: '.$pick_r[0].' -- '.$pick_r[1].' -- '.$pick_r[2], TICK_LOG_FILE_NPC);
			...
			if(!$db->query($update_res)) {
				$sdl->log('<b>Error:</b> Could not update Handelslager - '.$update_res, TICK_LOG_FILE_NPC);
			}
			else
			{
				...
				*/
				...
				if(!$db->query($sql)) {
					$sdl->log('<b>Error:</b> Could not update Ramona\'s planet - '.$sql, TICK_LOG_FILE_NPC);
				}
				...
				if($units = $db->queryrow($sql))
				{
					...
					{
						$sdl->log('Produce new units: '.$train_u[0].' -- '.$train_u[1].' -- '.$train_u[2].' -- '.$train_u[3].' -- '.$train_u[4].' -- '.$train_u[5], TICK_LOG_FILE_NPC);
...
/*						$sql='UPDATE planets SET
						...
						if(!$db->query($sql)) {
							$sdl->log('<b>Error:</b> Could not instruct Ramona to produce new units - '.$sql, TICK_LOG_FILE_NPC);
						}
					}
				}
				else
					$sdl->log('<b>Error:</b> Cannot read from Ramona\'s planet!', TICK_LOG_FILE_NPC);
			}
			$db->unlock('FHB_Handels_Lager');
		}

		$sdl->finish_job('Update Ramona resources svailability', TICK_LOG_FILE_NPC);

		// ########################################################################################
		// ########################################################################################
		// ########################################################################################
		//FHB_Sperrliste aufräumen
		$sdl->start_job('Remove card index corpse in the check list', TICK_LOG_FILE_NPC);
		$sql = "SELECT count(*) as anzahl,user_id FROM FHB_sperr_list GROUP By user_id";
		if(!$temps=$db->query($sql)) $sdl->log('<b>Error:</b> User query went wrong -- delete '.$sql, TICK_LOG_FILE_NPC);
		$anzahl_sperren=0; 
		while($result = $db->fetchrow($temps))
		{
			$sql_select=$db->query('SELECT user_id FROM user WHERE user_id='.$result['user_id']);
			if($db->num_rows($sql_select)<=0)
			{
				if(!$db->query('DELETE FROM FHB_sperr_list WHERE user_id='.$result['user_id'].''))
					$sdl->log('<b>Error:</b> Could not delete debts of dead user '.$sql, TICK_LOG_FILE_NPC);
				$sdl->log('Punishments of user: '.$result['user_id'].' deleted', TICK_LOG_FILE_NPC);
			}
		}
		$sdl->finish_job('Remove card index corpse in the check list', TICK_LOG_FILE_NPC);
		// ########################################################################################
		//Lernen ist langweilig hier das cheaten von ress durch truppen verkauf behoben
		$sdl->start_job('Soldier Transaction', TICK_LOG_FILE_NPC);
		$transaktionen=0;
		...
		while($cache_trade = $db->fetchrow($sql))
		{
			...
			if(!$db->query($update_action))$sdl->log('<b>Error:</b> Could not update Handelslager - '.$update_action, TICK_LOG_FILE_NPC);
			$db->unlock('FHB_Handels_Lager');
			$delete_action='DELETE FROM FHB_cache_trupp_trade WHERE id='.$cache_trade['id'].'';
			if(!$db->query($delete_action))$sdl->log('<b>Error:</b> Could not update Handelslager - '.$delete_action, TICK_LOG_FILE_NPC);
			
		}
		$sdl->log('Transactions: '.$transaktionen, TICK_LOG_FILE_NPC);
		$sdl->finish_job('Soldier Transaction', TICK_LOG_FILE_NPC);
		/*
		FHB_stats graph characters
		*/
		// ########################################################################################
		//Sensors monitoring and user warning
		$sdl->start_job('Sensors monitor', TICK_LOG_FILE_NPC);
		$anzahl_sesn_igms=0;
		...
		$angreifer=$db->query($angreifer);
		while($result = $db->fetchrow($angreifer))
		{
			$sdl->log('The User '.$result['user_id'].' will doch erhlich seine Fleet zerschrotten - noobs, alles noobs die ganze Gala und Tap ist ober Noob', TICK_LOG_FILE_NPC);
			$anzahl_sesn_igms++;
			$ziel_planet='SELECT planet_owner,planet_name FROM `planets` WHERE planet_owner ='.$result['user_id'].' LIMIT 0 , 1';
			$ziel_planet=$db->queryrow($ziel_planet);
			...
			{
				case 'GER':
					$text ='<br><center><b>Stellen Sie sofort den Angriff ein!</b></center>
					...
					$title = '<b>Sie sind auf unseren Sensoren</b>';
				break;
				case 'ENG':
					$text ='<br><center><b>Stop the attack immediately!</b></center>
						...
						<br>There are 5 small Light Attack Hunter on the way to your planet '.$ziel_planet['planet_name'].'.
						<br>--------------------------------------<br>
						Commander of Alpha-Fleet of the Trading Empire';
					$title = '<b>You are on our sensors</b>';
				break;
				case 'ITA':
					$text='<br><center><b>Fermate immediatamente il vostro attacco!</b></center>
						...
						<br>Ci sono 5 piccoli Caccia Leggeri d&#146;Attacco in rotta verso il vostro pianeta '.$ziel_planet['planet_name'].'.
						<br>--------------------------------------<br>
						Comandante dell&#146;Alpha-Fleet dell&#146;Impero Commerciale';
					$title = '<b>Siete sui nostri sensori</b>';
				break;
			}
			$this->MessageUser($this->bot['user_id'],$result['user_id'],$title,$text);
		}
		$sdl->log('Number of messages:'.$anzahl_sesn_igms, TICK_LOG_FILE_NPC);
		$sdl->finish_job('Sensors monitor', TICK_LOG_FILE_NPC);
		// ########################################################################################
		// ########################################################################################
		//Ships creation
		$sdl->start_job('Creating ships', TICK_LOG_FILE_NPC);
		$abfragen=$db->query('SELECT * FROM `ship_fleets` WHERE fleet_name="Alpha-Fleet IVX" and user_id='.$this->bot['user_id'].' LIMIT 0, 1');
		...
		if($db->num_rows()<=0)
		{
			...
			if(!$db->query($sql)) $sdl->log('<b>Error:</b> Could not insert new fleets data', TICK_LOG_FILE_NPC);
			...
			if(!$db->query($sql_x5)) $sdl->log('<b>Error:</b> Could not insert new fleets data', TICK_LOG_FILE_NPC);
			$fleet_id_x5= $db->insert_id();
			if(!$fleet_id) $sdl->log('Error - '.$fleet_id.' = empty', TICK_LOG_FILE_NPC);
			if(!$fleet_id_x5) $sdl->log('Error - '.$fleet_id.' = empty', TICK_LOG_FILE_NPC);
			$sql_a= 'SELECT * FROM ship_templates WHERE id = '.$this->bot['ship_t_1'];
			$sql_b= 'SELECT * FROM ship_templates WHERE id = '.$this->bot['ship_t_2'];
			if(($stpl_a = $db->queryrow($sql_a)) === false) $sdl->log('<b>Error:</b> Could not query ship template data - '.$sql_a, TICK_LOG_FILE_NPC);
			if(($stpl_b = $db->queryrow($sql_b)) === false) $sdl->log('<b>Error:</b> Could not query ship template data - '.$sql_b, TICK_LOG_FILE_NPC);
			...
			for($i = 0; $i < 4000; ++$i)
			{
				if($i<400){
					if(!$db->query($sql_c)) {
						$sdl->log('<b>Error:</b> Could not insert new ships #'.$i.' data', TICK_LOG_FILE_NPC);
					}
				}else{
					if(!$db->query($sql_d)) {
						$sdl->log('<b>Error:</b> Could not insert new ships #'.$i.' data', TICK_LOG_FILE_NPC);
			...
			$sdl->log('Fleet: '.$fleet_id.' - 2000 ships created', TICK_LOG_FILE_NPC);
		}
		$flotte=$db->fetchrow($abfragen);
		if($flotte['n_ships']<4000)
		{
			...
			if(!$db->query($sql)) $sdl->log('<b>Error:</b> Could not update new fleets data', TICK_LOG_FILE_NPC);
			$sql_b= 'SELECT * FROM ship_templates WHERE id = '.$this->bot['ship_t_2'];
			if(($stpl = $db->queryrow($sql_b)) === false) $sdl->log('<b>Error:</b> Could not query ship template data - '.$sql_b, TICK_LOG_FILE_NPC);
			...
			$sdl->log('Fleet: '.$flotte['fleet_id'].' -to '.$gebraucht.' ships updated', TICK_LOG_FILE_NPC);
		}
		$sdl->finish_job('Creating ships', TICK_LOG_FILE_NPC);

		// ########################################################################################
		// ########################################################################################

		$sdl->log('<b>Finished Scheduler in <font color=#009900>'.round((microtime()+time())-$starttime, 4).' secs</font>'."\n".'Executed Queries: <font color=#ff0000>'.$db->i_query.'</font></b>', TICK_LOG_FILE_NPC);
	}
